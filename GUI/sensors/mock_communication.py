...
class MockCommunication:
    def __init__(self):
        ...
        self.running = False
        self.thread = None
        print("MockCommunication initialized for development/testing")
    ...
    def register_callback(self, sensor_id, callback):
        ...
        print(f"Registered callback for sensor: {sensor_id}")
        if not self.running:
            self.start()
    ...
    def deregister_callback(self, sensor_id):
        ...
        if sensor_id in self.callbacks:
            ...
            print(f"Deregistered callback for sensor: {sensor_id}")
            ...
            try:
                ...
                for sensor_id, callback in self.callbacks.items():
                    if sensor_id == 'TEMP_SENSOR':
                        ...
                        temp = 20 + 10 * random.random() + 5 * random.sin(time.time() / 10)
    ...
    def close(self):
        ...
        self.stop()
        print("MockCommunication closed")